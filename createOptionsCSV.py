...
#Verification Window Format
#verify_text = sg.Text("Enter Verification Code: ")
#verify_layout = [[verify_text],[sg.InputText()]]
#verify_window = sg.Window("Verification Code",verify_layout)
...
while True:
    ...
    # presses Login button and continues with program
...
if main_window_active == True:
    while True:
        ...
        if main_event == "Create CSV File":
            try:
                robin_stocks.robinhood.export.export_completed_option_orders('.', file_name='allOptionOrders.csv')

                with open('allOptionOrders.csv', 'r') as updated_csv: # pass the file object to DictReader() to get the DictReader object
                    dict_reader = csv.DictReader(updated_csv) # get a list of dictionaries from dct_reader
                    updated_list  = list(dict_reader) # print list of dict i.e. rows
                for option in updated_list:
                    #Select short options from between selected dates and write to excel spreadsheet
